...
class ScoreDataMainUDF extends GenericUDF {
  private PrimitiveObjectInspector[] inFieldOI;
  GBM_Model p = new GBM_Model();
    ...
    if (record != null) {
      if (record.length == p.getNumCols()) {
        ...
        try {
          ...
          double[] result = (double[]) preds;
          Double[] finalPreds = new Double[result.length];
          for (int i = 0; i < result.length; i++) {
        	  finalPreds[i]=(Double.valueOf(result[i]));
		  }
          
          /**
          ArrayList<DoubleWritable> resultInts = new ArrayList<DoubleWritable>();
          for (int i = 0; i < preds.length; i++) {
        	  resultInts.add(new DoubleWritable((preds[i])));
          }
          **/
          /**
          ArrayList<Text> resultTexts = new ArrayList<Text>();
          for (int i = 0; i < preds.length; i++) {
        	  resultTexts.add(new Text(String.valueOf(preds[i])));
          }
          **/
